...
def pretty_english(text):
    last=len(text)
    while True:        
        text=re.sub(REGEXES['prettyEnglish'],' ',text)
        if len(text)==last:break
        last=len(text)
    return text
...
class Document:
    ...
    def __init__(self, input, **options):
        ...
        kwargs:
            ...
            - description_length: will generate desciption
    ...
    def description(self):
        if not hasattr(self,'_plain_article'):
            self.plain_article()
        return self._plain_article[:200]

    def plain_article(self):
        if not hasattr(self,'_article'):
            self.summary()
        tagged_article=self._parse(self._article)
        self._plain_article=tagged_article.text_content()
        return self._plain_article
    ...
    def summary(self, html_partial=True,pretty=True):
        ...
        try:
            ...
            while True:
                ...
                if ruthless and not of_acceptable_length:
                    ...
                    if pretty:
                        self._article=pretty_english(cleaned_article)
                    else:
                        self._article=cleaned_article
                    return self._article
...
#                    for img in imgs:
#
#                        hThe University of Central Lancashire in collaboration with Nasa, the   Harvard-Smithsonian Center for Astrophysics and other partners have captured   the highest resolution images ever achieved of the Sun's outer atmosphere   (corona).</p><p>Nasa's High Resolution Coronal Imager, or Hi-C, was launched to the edge of   space on July 11 2012, from White Sands Missile Range in New Mexico, USA.</p><p>For several decades scientists have sought to understand why the outer parts   of the solar atmosphere are on average about two million degrees Kelvin,   about 400 timeseight = img.get('height')
...
#                            allowed[desnode] = True
        ...
        for el in ([node] + [n for n in node.iter()]):
            if not self.options.get('attributes', None):
                ...
                pass
        node,img_list=self.mine_convert(node)
        self.img_list=img_list
        #eturn clean_attributes(tounicode(node))
        article=self.mine_clean(node.text_content(),img_list)

        return article

    def mine_convert(self,node):
        """ deal with p """
        from lxml import etree
        img_list=[]
        for para in node.xpath("*//p"):
            text=para.text.strip() if para.text  else ''
            if not para.getchildren() and not text:
                para.getparent().remove(para)
            para.text="%s[[para]]" %text

        for index,img in enumerate(node.xpath("*//img")):
            img_list.append(img.get('src'))
            img.tail='[[image %s]]' %str(index)

        etree.strip_elements(node,'img',with_tail=False)
        #print node.text_content()
        """ deal with image """
        return node,img_list

    def mine_clean(self,article,img_list):
        """remove \n\t\r  """
        article=(re.sub('\\n|\\t|\\r','',article)).strip()
        if article.startswith('[[para]]'):article=article[8:]
        if article.endswith('[[para]]'):article=article[:-8]
        article=re.sub('\[\[para\]\]','</p><p>',article)
        article=re.sub('<p></p>','',article)
        article=''.join(['<p>',article,'</p>']) if article else article        
        def img_repl(matchobj):
            num=matchobj.group(1)
            return ''.join(["<img id='art_img_",num,"'"," class='art_img' src='",img_list[int(num)],"'/>"])
        article=re.sub(r'\[\[image (\d+)\]\]',img_repl,article)
        return article
