...
class SessionManager(object):
    ...
    SESSION_ID_NAME = 'YOULIL_ID'
    STORAGE_CATEGORY = 'db_sessions'

    driver = None

    def __init__(self, handler):#get_secure_cookie, set_secure_cookie):
        '''
    ...
    def __setup_settings(self):
        pycket_settings = self.handler.settings.get('session_config')
        if not pycket_settings:
            raise ConfigurationError('The "session_config" configurations are missing')
